...
static void echo_line( void )
{
  ...
  if ( ios_line_ready ) {
    printf("%s\n",&(ios_line[ ios_line_disp ]));
    return;
  }
  if ( ios_line_idx <= ios_line_disp ) return;
  printf("%s",  &(ios_line[ ios_line_disp ])); 
  fflush( stdout );
...
static void parse_move( void )
{
   if ( toupper(ios_line[57]) == 'P' ) 
      v_ios.u.BOARD.last_move = MY_PASS;
   else
...
static void parse_CREATE( void )
{
  ...
  for (i = 0; ios_line[i++] != '('; );
  for (j=i;ios_line[j] != ' ';j++)
    v_ios.u.CREATE.black[j-i]=ios_line[j];
  v_ios.u.CREATE.black[j-i]=0;

  for (i=j+5;ios_line[i] != ')';i++)
    v_ios.u.CREATE.white[i-j-5]=ios_line[i];
  v_ios.u.CREATE.white[i-j-5]=0;
